...
export const JSPromises = () => {
  ...
  const demoStaticMethods = () => {
    ...
    const thenable = {
      then: (resolve: (value: string) => void) => resolve("From thenable"),
    };
  ...
  const promsie = (attempts: number, maxAttempts: number) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
  ...
  const retryPromiseMaxReached = async (
    attempts: number,
  ...
  const retryPromiseSuccess = async (
    attempts: number,
    maxAttempts: number = 3,
  ) => {
    // const maxAttempts = 3;
  ...
  const promiseCopy = new MyPromiseCopy(function (_resolve: any, _reject: any) {});
  ...
  (reason) => {
    console.error(reason); // Error!
      ...
      <section className="grid gap-6">
        ...
        >
          <Button
            label="Retry Promise Max fail"
            handleClick={() => retryPromiseMaxReached(0)}
          />
          <Button
            label="Retry Promise success"
            handleClick={() => retryPromiseSuccess(0)}
          />
        </Grid>
      </section>
    </>
...
class MyPromiseCopy {
  ...
  #handlers: Array<{ onFulfill: (value: any) => void; onReject: (error: any) => void }> = [];
  #value: any = null;
  #error: any = null;

  constructor(callback: (resolve: (value: any) => void, reject: (error: any) => void) => void) {
  ...
  then(onFulfill?: (value: any) => any, onReject?: (error: any) => any): MyPromiseCopy {
    return new MyPromiseCopy((resolve: (value: any) => void, reject: (error: any) => void) => {
      const handler = {
        onFulfill: (value: any) => {
          try {
  ...
  catch(onReject: (error: any) => any): MyPromiseCopy {
    return this.then(undefined, onReject);
  }

  finally(callback: () => void): MyPromiseCopy {
    return this.then(
      (value) => {
        callback();
        return value;
